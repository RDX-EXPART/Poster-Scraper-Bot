...
class EchoBypass:
    ...
    def _norm(self, data):
        ...
        if not isinstance(root, dict):
            LOGGER.error(f"[{self.key}] Unexpected root structure after normalization")
            return None, "Unexpected response from bypass service."

        direct = (
            root.get("url")
            or root.get("link")
            or root.get("download")
            or root.get("direct_link")
        )
        ...
        if isinstance(direct, str) and direct.startswith(("http://", "https://")):
            return {
                ...
                "format": root.get("format") or root.get("file_format") or "N/A",
...
def _xlnk(root):
    ...
    if isinstance(direct, str) and direct.startswith(("http://", "https://")):
        out["Direct Link"] = direct

    for k, v in root.items():
        if not isinstance(v, dict):
            continue

        url = v.get("link") or v.get("url") or v.get("download")
    ...
    if isinstance(raw, dict):
        for k, v in raw.items():
            if isinstance(v, str) and v.startswith(("http://", "https://")):
                ...
                u = v.get("url") or v.get("link") or v.get("download")
        ...
        for i in raw:
            if not isinstance(i, dict):
                continue
            u = i.get("url") or i.get("link") or i.get("download")
...
def _detect_service_from_url(url):
    try:
        p = urlparse(url)
        host = p.netloc.lower()
        path = p.path.lower()

        if "hubcloud.foo" in host and path.startswith("/drive/"):
            return "hubcloud"

        if "gdflix.dev" in host and path.startswith("/file/"):
            return "gdflix"

        if "hubdrive" in host:
            return "hubdrive"

        if "extraflix" in host:
            return "extraflix"
...
async def _bpinfo(cmd_name, target_url):
    srv = _bysrv(cmd_name)
    if not srv:
        auto_key = _detect_service_from_url(target_url)
        if auto_key:
            srv = EchoByRegistry.get(auto_key)
    if not srv:
